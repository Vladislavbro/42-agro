...
- "Культура": Определи полное название культуры из "СПИСКА КУЛЬТУР", основываясь на тексте, связанном с этой комбинацией (например, "оз пш" -> "Пшеница озимая"). Если культуране указана, не распознана, или этой  культуры нет в словаре, используй null.
...
- "Вал за день, ц": Валовый сбор за день в центнерах. В сообщениях присылают в килограммах, если не указано явно центнеры. Надо перевести в центнеры. Если не указано, используй null.
- "Вал с начала, ц": Валовый сбор с начала операции в центнерах. В сообщениях присылают в килограммах, если не указано явно центнеры. Надо перевести в центнеры. Если не указано, используй null.
...
По Пу 26/488
Отд 12 26/221
То бери информацию только по ПУ - так как это данные сразу по производственному участку
